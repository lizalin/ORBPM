<?php
	...
	if($isPaging==0 && $_REQUEST['hdn_PageNo']=='' && $_REQUEST['ID']>0)
	{
        ...
        $intCategory	  =0;
        $txtHeadlineE         = (isset($_REQUEST['txtheadE'])&& $_REQUEST['txtheadE']!='')?trim(htmlspecialchars($_REQUEST['txtheadE'],ENT_QUOTES)):'';
       
        //============= search function =================
         if(isset($_REQUEST['btnSearch']))
        {
            //echo $txtHeadlineE; exit;
            $intPgno	= 1;
            $intRecno	= 0;
        }
	...
	if(isset($_REQUEST['hdn_qs'])&& $_REQUEST['hdn_qs']!='')
	{
        ...
        if($isPaging==0){	
            $result		= $obj->manageImpServices('PG',$intRecno,$intCategory,0,0,0,0,$txtHeadlineE,'','','','','',0,0,0);
            //print_r($result);
        }else
	{
            $intPgno	= 1;
            $intRecno	= 0;
            $result                 = $obj->manageImpServices('V',0,$intCategory,0,0,0,0,$txtHeadlineE,'','','','','',0,0,0);
           
	}
       $totalResult                 = $obj->manageImpServices('V',0,$intCategory,0,0,0,0,$txtHeadlineE,'','','','','',0,0,0);      
