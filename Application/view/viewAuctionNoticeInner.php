   ...
   if($_REQUEST['hdn_IsPaging']!="" && $_REQUEST['hdn_IsPaging'] >0)
	...
	if($isPaging==0 && $_REQUEST['hdn_PageNo']=='' && $_REQUEST['ID']>0)
	{
        ...
        //============= search function =================
         ...
         $intRTOType	  = (isset($_REQUEST['RTOType'])&& $_REQUEST['RTOType']!='')?trim(htmlspecialchars($_REQUEST['RTOType'],ENT_QUOTES)):'0';
        //$intSection	  = (isset($_REQUEST['noticeType'])&& $_REQUEST['linkType']!='')?trim(htmlspecialchars($_REQUEST['noticeType'],ENT_QUOTES)):'0';
        $txthead         = (isset($_REQUEST['txthead'])&& $_REQUEST['txthead']!='')?trim(htmlspecialchars($_REQUEST['txthead'],ENT_QUOTES)):'';
	...
	if(isset($_REQUEST['hdn_qs'])&& $_REQUEST['hdn_qs']!='' )
	{
        ...
        if($isPaging==0)	
            $result		= $obj->manageAuctionNotice('PG',$intRecno,0,$intRTOType,0,0,0,0,'',$txthead,'','','','','','0000-00-00','0000-00-00',0,2,0,0,0);
       
	else
	{
            $intPgno	= 1;
            $intRecno	= 0;
            $result                 = $obj->manageAuctionNotice('V',0,0,$intRTOType,0,0,0,0,'',$txthead,'','','','','','0000-00-00','0000-00-00',0,2,0,0,0);      
	}
        /*variable to count total records in common table t_notification*/
        $totRecords                 = $obj->manageAuctionNotice('V',0,0,0,0,0,0,0,'','','','','','','','0000-00-00','0000-00-00',0,2,0,0,0);
        $totRecords                 = $totRecords->num_rows;  
       $totalResult                 = $obj->manageAuctionNotice('V',0,0,$intRTOType,0,0,0,0,'',$txthead,'','','','','','0000-00-00','0000-00-00',0,2,0,0,0);    
