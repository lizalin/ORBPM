<?php
	...
	if($isPaging==0 && $_REQUEST['hdn_PageNo']=='' && $_REQUEST['ID']>0)
	{
        ...
        $selplugintype	  = (isset($_REQUEST['selCategory'])&& $_REQUEST['selCategory']!='')?$_REQUEST['selCategory']:'0';
	...
	if(isset($_REQUEST['hdn_qs'])&& $_REQUEST['hdn_qs']!='')
	{
		...
		$outMsg	= $obj->deleteActRules($qs,$ids);
	}
        if($isPaging==0)	
            $result		= $obj->manageActRules('PG',$intRecno,$selplugintype,$txthead,'','',0,2,0,0);              
	else
	{
            $intPgno	= 1;
            $intRecno	= 0;
            $result                 = $obj->manageActRules('V',0,$selplugintype,$txthead,'','',0,2,0,0);      
	}
       $totalResult                 = $obj->manageActRules('V',0,$selplugintype,$txthead,'','',0,2,0,0);      
